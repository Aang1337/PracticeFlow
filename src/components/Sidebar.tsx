...
export default function Sidebar({
  ...
  const renderList = (videos: VideoFile[], title: string, showUpload = false) => (
    <div className="mb-6">
      <div className="flex items-center justify-between px-5 mb-2">
        ...
        {showUpload && (
          <button
            ...
            className="text-[10px] font-bold hidden xl:inline text-violet-400 hover:text-violet-300 opacity-60 hover:opacity-100 cursor-pointer"
          >
            + Upload
          </button>
        )}
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {video.file && (
                     <span className="text-[9px] text-emerald-400/50">Uploaded File</span>
                  )}
                </div>
      ...
      {showUpload && videos.length === 0 && (
         <button
            onClick={handleUploadClick}
            className="mx-5 mt-2 px-3 py-1.5 rounded-lg border border-dashed border-white/20 text-xs text-white/40 hover:text-white/80 hover:bg-white/5 transition-all text-center block w-[calc(100%-40px)]"
         >
  ...
  return (
    <aside className="hidden lg:flex flex-col w-80 border-l border-white/5 bg-zinc-900/50 backdrop-blur-xl h-full shadow-[-20px_0_40px_rgba(0,0,0,0.2)]">
      <div className="p-5 border-b border-white/5 bg-black/20">
        ...
        <p className="text-[10px] text-white/40">Select or upload a video</p>
      </div>

      <div className="flex-1 overflow-y-auto scrollbar-thin py-4">
        {renderList(localVideos, 'Local Videos', true)}
